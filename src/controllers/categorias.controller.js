...
const categoriaController = {
    ...
    criar: async (req, res) => {
        try {
            ...
            const result = await categoriaModel.cadastrarCategoria({ descricaoCategoria });
            
            if (result.insertId > 0) return res.status(201).json({ message: "Categoria cadastrada com sucesso" });
    ...
    atualizar: async (req, res) => {
        try {
            const id = req.params.id;
            const { descricaoCategoria } = req.body;
            const result = await categoriaModel.atualizarCategoria(id, descricaoCategoria);
            if (result.affectedRows > 0) return res.status(200).json({ message: "Categoria atualizada com sucesso" });
    ...
    excluir: async (req, res) => {
        try {
            ...
            if (result.affectedRows > 0) return res.status(200).json({ message: "Categoria excluída com sucesso" });
